        ...
        for seq in seqs:
            if not ortho6d:
                self.train_sets.append(
                    KittiOdometryRandomSequenceDataset(seq, path=path)
                )
            else:
                self.train_sets.append(
                    KittiOdometryRandomSequenceDataset6D(seq, path=path)
                )
        self.idxs = []
        self.seps = []
...
# orthonormal 6D
class KittiOdometryRandomSequenceDataset6D(Dataset):
    def __init__(
        self,
        seq,
        path=KITTI_PATH,
    ...
    def __getitem__(self, idx):
        ...
        if not POSE:
            ...
            for i in range(SEQ_LENGTH):
                rgb.append(
                    torch.cat(
                        (
                            self.normalizer(
                                self.transform(self.odom.get_rgb(idx + i)[left]) - 0.5
                            ),
                            self.normalizer(
                                self.transform(self.odom.get_rgb(idx + i + 1)[left])
                                - 0.5
                            ),
                        ),
                        dim=0,
            ...
            rgb = torch.stack(rgb)
            if HALF:
                return [rgb.type(torch.HalfTensor)]
            else:
                return [rgb]
        if torch.is_tensor(idx):
            idx = idx.to_list()
        ...
        pos = []
        angle = []
        original_angle = torch.FloatTensor(se3_to_rot(self.odom.poses[idx]))
        original_pos = torch.FloatTensor(se3_to_position(self.odom.poses[idx]))
        original_rot_inv = se3_to_rot(self.odom.poses[idx]).T
        pos.append(original_pos)
        angle.append(original_angle)
        for i in range(SEQ_LENGTH):
            next_pos = torch.FloatTensor(se3_to_position(self.odom.poses[idx + i + 1]))
            ...
            rgb.append(
                torch.cat(
                    (
                        ...
                        self.normalizer(
                            self.transform(self.odom.get_rgb(idx + i + 1)[left]) - 0.5
                        ),
                    ),
                    dim=0,
                )
        ...
        pos[1:] = pos[1:] - original_pos
        for i in range(1, len(pos)):
            loc = torch.FloatTensor(original_rot_inv.dot(pos[i]))
            pos[i][:] = loc[:]
        pos[2:] = pos[2:] - pos[1:-1]

        for i in range(1, len(angle)):
            angle[i] = original_rot_inv.dot(angle[i])
        for i in range(len(angle) - 1, 1, -1):
            angle[i] = angle[i - 1].T @ angle[i]
        for i in range(len(angle)):
            angle[i] = torch.FloatTensor(ortho6d_from_matrix(angle[i]).flatten("F"))
        angle = torch.stack(angle)
        if HALF:
            return (
                rgb.type(torch.HalfTensor),
...
# SE3 and rot mat
def ortho6d_from_matrix(matrix):
    return np.array(matrix, dtype=np.float64, copy=False)[:3, :2]
...
def test_rot(model, dataloader, device, epoch, path=KITTI_PATH, test_seq=TEST_SEQ):
    model.eval()
    traj = []
    Rs = []
    cur_R = np.eye(3)
    ...
    for i, batch in enumerate(dataloader):
        ...
        predicted = model(seq)
        predicted = predicted[:, -1, :]
        predicted = compute_rotation_matrix_from_ortho6d(predicted)
        predicted = predicted.data.cpu().numpy()
        if i == 0:
            Rs.append(cur_R.copy())
            traj.append(np.concatenate((cur_R.copy(), cur_t.copy()), axis=1).flatten())
            predicted = predicted[1:]
        for rot in predicted:
            cur_R = rot @ cur_R
            Rs.append(cur_R.copy())
            traj.append(np.concatenate((cur_R.copy(), cur_t.copy()), axis=1).flatten())
...
def test_rot6d(
    ...
    traj.append(np.concatenate((cur_R.copy(), cur_t.copy()), axis=1).flatten())
    for i, batch in enumerate(dataloader):
        seq = batch[0] if pose else batch
        seq = seq.to(device)
        predicted = model(seq)
        if i == 0:
            for rot in predicted[0, :, :]:
                rot = torch.unsqueeze(rot, dim=0)
                rot = (
                    compute_rotation_matrix_from_ortho6d(rot, half=HALF)
                    .data.cpu()
                    .numpy()
                    .squeeze()
                )
