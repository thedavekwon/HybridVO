...
# batch*n
def normalize_vector(v, return_mag=False, half=False):
    batch = v.shape[0]
    ...
    if not half:
        ...
        v_mag = torch.max(
            ...
            torch.autograd.Variable(
                torch.FloatTensor([1e-8]).type(torch.HalfTensor).cuda()
            ),
        )
...
# poses batch*6
# poses
def compute_rotation_matrix_from_ortho6d(ortho6d, half=False):
    x_raw = ortho6d[:, 0:3]  # batch*3
    y_raw = ortho6d[:, 3:6]  # batch*3

    x = normalize_vector(x_raw, half=half)  # batch*3
    z = cross_product(x, y_raw)  # batch*3
    z = normalize_vector(z, half=half)  # batch*3
...
def compute_rotation_matrix_from_ortho5d(a):
    ...
    proj_scale = (
        torch.autograd.Variable(torch.FloatTensor(proj_scale_np).cuda())
        .view(1, 3)
        .repeat(batch, 1)
    )  # batch,3
...
def get_sampled_rotation_matrices_by_hpof(batch):
    ...
    theta = torch.autograd.Variable(
        torch.FloatTensor(np.random.uniform(0, 1, batch) * np.pi).cuda()
    )  # [0, pi]
    phi = torch.autograd.Variable(
        torch.FloatTensor(np.random.uniform(0, 2, batch) * np.pi).cuda()
    )  # [0,2pi)
...
def get_sampled_rotation_matrices_by_axisAngle(batch, return_quaternion=False):
    ...
    theta = torch.autograd.Variable(
        torch.FloatTensor(np.random.uniform(-1, 1, batch) * np.pi).cuda()
    )  # [0, pi] #[-180, 180]
    sin = torch.sin(theta)
