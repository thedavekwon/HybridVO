...
class VO3D2D(VO):
    ...
    def update(
        ...
        vel = None
        if not idx:
            tpts, left_indexes, right_indexes, left_kps, left_ds, right_kps, right_ds, matches = self.triangulate(
                left_img, right_img
            )
        else:
            prev_tpts, prev_left_indexes, _, _, prev_left_ds, _, _, _, _, _, prev_vel = self.cache[
                -1
            ]
            ...
            if prev_vel and vel < 2 * prev_vel:
...
class VO3D3D(VO):
    ...
    def update(
        ...
        if not idx:
            ...
            prev_tpts, prev_left_indexes, _, _, prev_left_ds, _, _, _, _, _ = self.cache[
                -1
            ]
